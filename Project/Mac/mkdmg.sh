...
if [ "$KIND" = "CLI" ]; then
    ...
    cp "../../Source/ThirdParty/c2pa-rs/target/release/libc2pa_c.dylib" "${FILES}-Root/usr/local/lib/${APPNAME_lower}/lib"
    codesign -f --deep --options=runtime -s "Developer ID Application: ${SIGNATURE}" --verbose "${FILES}-Root/usr/local/lib/${APPNAME_lower}/lib/libc2pa_c.dylib"
...
if [ "$KIND" = "GUI" ]; then
    ...
    mkdir -p "${FILES}/${APPNAME}.app/Contents/Frameworks"
    cp "../../Source/ThirdParty/c2pa-rs/target/release/libc2pa_c.dylib" "${FILES}/${APPNAME}.app/Contents/Frameworks"
    codesign -f --options=runtime -s "Developer ID Application: ${SIGNATURE}" --verbose "${FILES}/${APPNAME}.app/Contents/Frameworks/libc2pa_c.dylib"
