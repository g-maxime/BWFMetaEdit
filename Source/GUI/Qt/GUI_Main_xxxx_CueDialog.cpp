...
void CueDialogCombo_Delegate::setEditorData(QWidget* editor, const QModelIndex& index) const
{
    ...
    if (index.column()==Column_Country)
    {
        ...
        codes CSETCountries=CSETCountries_Get();
        for (codes::iterator It=CSETCountries.begin(); It!=CSETCountries.end(); It++)
        {
        ...
        codes ISOCountries=ISOCountries_Get();
        for (codes::iterator It=ISOCountries.begin(); It!=ISOCountries.end(); It++)
        {
            QString Entry=QString().fromStdString(CodeToCountry(It->first));
            MinWidth=max(MinWidth, Editor->fontMetrics().size(Qt::TextSingleLine, Entry).width()+20);
    ...
    else if (index.column()==Column_Language)
    {
        ...
        Editor->addItem("");
        codes CSETLanguages=CSETLanguages_Get();
        for (codes::iterator It=CSETLanguages.begin(); It!=CSETLanguages.end(); It++)
        {
        ...
        codes ISOLanguages=ISOLanguages_Get();
        for (codes::iterator It=ISOLanguages.begin(); It!=ISOLanguages.end(); It++)
        {
            QString Entry=QString().fromStdString(CodeToLanguage(It->first));
            MinWidth=max(MinWidth, Editor->fontMetrics().size(Qt::TextSingleLine, Entry).width()+20);
    ...
    else if (index.column()==Column_Dialect)
    {
        ...
        Editor->addItem("");
        codes Dialects=CSETDialects_Get();

        for (codes::iterator It=Dialects.begin(); It!=Dialects.end(); It++)
        {
